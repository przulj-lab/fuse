% Function for creating block matrices from edgelists
% -------------------------------------------------------------------------------------------------------------
...
% --------------------------------------------------------------------------------------------------------------
% [Input]:
%     adj_list: <string cell array>, file names of networks given in the edgelist
...
for ii=1:length(adj_list)
    ...
    A{ii} = full(A{ii} + A{ii}'); % symmetric adjacency matrix
    s = size(A{ii});
    ...
    fprintf('Loading network %s finished!\n',net_name);
